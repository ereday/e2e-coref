...
#  pronoun,A,B     = row['Pronoun'],row['A'],row['B']
  word_offsets = get_offsets(row[2])
  poff,aoff,boff  = int(row[8]),int(row[4]),int(row[6])
  pronoun,A,B     = row[7],row[3],row[5]
...
if __name__ == "__main__":
  ...
  with tf.Session() as session:
    ...
    for index,row in enumerate(df):
      ...
      example_new = {}
      example_new['sentences'] = example['sentences']
      example_new['predicted_clusters'] = example['predicted_clusters']
      pronoun,A,B     = row[2],row[4],row[7]
      example_new['pronoun'] = pronoun
      example_new['A'] = A
      example_new['B'] = B
